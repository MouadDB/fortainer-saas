...
const AcceptNodeInvitation: NextPageWithLayout = () => {
  ...
  const acceptInvitation = async () => {
    const response = await fetch(
      `/api/nodes/${invitation.node.slug}/invitations`,
      {
        method: 'PUT',
  ...
  return (
    <>
      <Head>
        <title>{`${t('invitation-title')} ${invitation.node.name}`}</title>
      </Head>
      <div className="rounded p-6 border">
        <div className="flex flex-col items-center space-y-6">
          <h2 className="font-bold">
            {`${invitation.node.name} ${t('node-invite')}`}
          </h2>
...
AcceptNodeInvitation.getLayout = function getLayout(page: ReactElement) {
...
export default AcceptNodeInvitation;
