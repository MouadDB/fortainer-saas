...
import { addNodeMember, getNode } from 'models/node';
    ...
    callbacks: {
      async signIn({ user, account, profile }) {
        ...
        if (!existingUser) {
          ...
          if (isIdpLogin && user) {
            await linkToNode(user as unknown as Profile, newUser.id);
          }

          if (account.provider === 'boxyhq-saml' && profile) {
            await linkToNode(profile, newUser.id);
          }
...
const linkToNode = async (profile: Profile, userId: string) => {
  ...
  let userRole: Role = node.defaultRole || Role.MEMBER;
  ...
  await addNodeMember(node.id, userId, userRole);
};
